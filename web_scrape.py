...
def get_prcnt_change(ticker, data):
    ...
    # c_time = c_time - datetime.timedelta(minutes=c_time.minute % 15)
    # c_time = c_time.replace(second=0)
    # replace when not testing
    c_time = c_time.replace(hour=14, minute=15, second=0)
    c_time_text = c_time.strftime("%Y-%m-%d %H:%M:%S")

    # getting current time and date in certain format - 15 mins
    l_time = c_time - datetime.timedelta(minutes=15)
